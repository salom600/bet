...
namespace {
inline double rat2d(const AVRational& r) {
...
FFmpegBackend::~FFmpegBackend() { close(); }
...
// Probe
// ---------------------------------------------------------------------------
MediaInfo FFmpegBackend::probe(const QString& path) {
    MediaInfo info;
    AVFormatContext* fmt = nullptr;
    const std::string p = path.toUtf8().constData();

    if (avformat_open_input(&fmt, p.c_str(), nullptr, nullptr) < 0) return info;
    ...
    for (unsigned i = 0; i < fmt->nb_streams; ++i) {
        ...
        if (par->codec_type == AVMEDIA_TYPE_VIDEO && !info.hasVideo) {
            ...
            if (st->avg_frame_rate.den) info.fps = rat2d(st->avg_frame_rate);
...
bool FFmpegBackend::open(const QString& path) {
    ...
    if (avformat_open_input(&fmtCtx_, p.c_str(), nullptr, nullptr) < 0) {
        qWarning() << "FFmpegBackend: cannot open" << path;
        return false;
    }
...
// ---------------------------------------------------------------------------
QImage FFmpegBackend::grabFrame(double atSeconds, int maxWidth, int maxHeight) {
    ...
    while (av_read_frame(fmtCtx_, pkt) >= 0) {
        ...
        if (avcodec_send_packet(videoCodec_, pkt) < 0) {
            av_packet_unref(pkt);
            continue;
        }
        av_packet_unref(pkt);
        ...
        while (avcodec_receive_frame(videoCodec_, frame) == 0) {
            ...
            if (lastSwsW_ != w || lastSwsH_ != h ||
                lastSwsFW_ != frame->width || lastSwsFH_ != frame->height ||
                lastSwsFmt_ != static_cast<int>(srcFmt)) {
                if (swsCtx_) sws_freeContext(swsCtx_);
                swsCtx_ = sws_getContext(frame->width, frame->height, srcFmt,
                                         w, h, AV_PIX_FMT_RGB24,
                                         SWS_BILINEAR, nullptr, nullptr, nullptr);
                lastSwsW_  = w; lastSwsH_ = h;
                lastSwsFW_ = frame->width; lastSwsFH_ = frame->height;
                lastSwsFmt_ = static_cast<int>(srcFmt);
            }
            if (swsCtx_) {
...
// ---------------------------------------------------------------------------
std::vector<float> FFmpegBackend::audioPeaks(int peaksCount) {
    ...
    if (swr_init(swrCtx_) < 0) return peaks;
    ...
    if (totalSamples <= 0) totalSamples = peaksCount * 256;
